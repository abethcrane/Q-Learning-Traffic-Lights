...
public class LearningModuleImpl implements LearningModule
{
	...
	@Override
	public List<Boolean> updateTrafficLights(RoadMap r, List<TrafficLight> trafficLights, int timeRan) {
	...
	@Override
	public List<Boolean> updateTrafficLightsRandomly(RoadMap mapWithCars, List<TrafficLight> trafficLights)
	{
	...
	public void learn(List<Integer> pastStates, List<Boolean> switches, List<Integer> rewards, List<Integer> newStates, List<TrafficLight> lights) {
		...
		for (int i = 0; i < lights.size(); i++) {
			//if (lights.get(i).getDelay() == 0) {
				...
				qValues.put((state + actionPosition*action.actionInt()), newQValue);
			//}
		}
	}
